...
{    
    ...
    f[0]=cosf(y[3])/sqrtf(s2);
    f[1]=-sinf(y[3])*cosf(y[4])/sqrtf(s2);
	f[2]=-sinf(y[3])*sinf(y[4])/sqrtf(s2);
	float dpdt;
	dpdt=-cosf(y[3])*((sds[0]/sqrtf(s2))/sqrtf(s2))
		+sinf(y[3])*((sds[1]/sqrtf(s2))/sqrtf(s2)*cosf(y[4])
					+(sds[2]/sqrtf(s2))/sqrtf(s2)*sinf(y[4]));
	if (y[3]==0) f[3] = 0.0;						  /*Ray do not change inclination direction*/
	else
	f[3]= -(sds[0]/s2+cosf(y[3])*dpdt)/(sqrtf(s2)*sinf(y[3]));
	if (sinf(y[3])==0.0 || sinf(y[4]==0)) f[4] = 0.0; /*Ray do not change azimuth direction*/
	else 
	f[4]= -(sds[1]/s2-(cosf(y[4])*sinf(y[3])*dpdt-cosf(y[4])*cosf(y[3])*sqrtf(s2)*f[3]))
		/(sqrtf(s2)*sinf(y[3])*(-sinf(y[4])));
}
...
int mtrace_ray (raytrace rt  /* ray tracing object */, 
 ...
 
{
    ...
    if (!rt->sym) {
		...
		it = ode23_step (y, rt,iso_rhs,term,traj,horizon,nhor,dhor,ohor);
		runge_close();
